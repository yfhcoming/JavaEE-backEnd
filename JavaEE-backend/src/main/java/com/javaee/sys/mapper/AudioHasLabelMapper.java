...
public interface AudioHasLabelMapper extends BaseMapper<AudioHasLabel> {
    ...
    @Select("select A.audio_id,A.audio_name,A.score,A.des,U.user_name,A.create_time from audio_has_label as H join audio as A on H.audio_id=A.audio_id join label as L on H.label_id=L.label_id left join user as U on A.user_id =U.user_id where L.label_id=#{LabelId}")
